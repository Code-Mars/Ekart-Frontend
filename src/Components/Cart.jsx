        ...
        <div className="md-mx:w-[95%] mx-auto w-[62%] p-3 m-2 border-[1px] border-gray-300 rounded-lg  flex flex-col gap-3">
            ...
            { items.length?
                items.map((item, index) => {
                    return <CartItem key={index} id={item.id} title={item.title} desc={item.desc} price={item.price} rating={item.rating} quantity={item.quantity} cart={true} />}):<div className="text-gray-600 text-lg font-semibold h-full flex items-center justify-center">Your cart is empty</div>
            }
        </div>
